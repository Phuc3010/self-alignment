...
from utils import rank0_print, on_rank0, delete_dict, load_jsonl_file
import pandas as pd
...
def get_synthetic(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str, file_path="generated/iter0") -> Dataset:
    if split == 'test':
        path = file_path + "/test.jsonl"
    elif split=="train":
        path = file_path + "/train.jsonl"
    else:
        raise ValueError(f'{split} is not found')

    rank0_print(f'Loading Synthetic dataset ({split} split) from Local...')

    dataset = load_jsonl_file(path)
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing Synthetic data')
    data = Dataset('synthetic')

    for row in dataset:
        prompt = human_prefix + row['prompt'] + human_suffix + assistant_prefix
        data[prompt].prompt = prompt
        data[prompt].generations.extend(row['generations'])
        data[prompt].desirable.extend(row['desirable'])
...
def get_ultrachat(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    if split == "train":
        split = "train_sft"
    elif split == "test":
        split = "test_sft"
    ...
    for row in dataset:
        ...
        data[prompt].desirable.append(True)
        data[prompt].original_prompt = row['prompt']
        data[prompt].truncation_mode = 'keep_start'
        data[prompt].remove_extra_spaces()
        # keep original prompt so that it can be dumped into a JSON file before running the alpacaeval command
        ...
        for prompt in prompts:
            ...
            if len(example.pairs) > 0:
                ...
                for i in range(len(example.desirable)):
                    # if seen_desirable < allowed_desirable and example.desirable[i]==True:
                    if example.desirable[i]==True:
                        flat_data.append((example, example.generations[i], 'chosen'))
                        # seen_desirable += 1
                    
                    # if seen_undesirable < allowed_undesirable and example.desirable[i]==False:
                    if example.desirable[i]==False:
...
if __name__=="__main__":
    ...
    dataloader = UnpairedPreferenceDataLoader(
        ["synthetic"], tokenizer, batch_size=32, n_examples=300, n_epochs=1, split="train"
    )
    eval_loader = UnpairedPreferenceDataLoader(
        ["synthetic"], tokenizer, batch_size=32, n_examples=300, n_epochs=1, split="test"
    )
    policy = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-160m-deduped", torch_dypte=torch.bfloat16, device_map="auto")
    config = OmegaConf.load("config/config.yaml")
    reference_model = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-160m-deduped", torch_dypte=torch.bfloat16, device_map="auto")
    trainer = KTOTrainer(
        tokenizer, 
        config,
        dataloader,
    ...
    for batch in dataloader:
        ...
        chosen_batch = {k:v[chosen_idx,...] for k, v in batch.items() if torch.is_tensor(v)}
        unlabeled_batch = {k:v[unlabeled_idx, ...] for k, v in batch.items() if torch.is_tensor(v)}
