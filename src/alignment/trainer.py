...
class KTOTrainer(Trainer):
    def __init__(
        ...
        if ref_model:
            ...
            # The `model` with adapters turned off will be used as the opponent model
            self.ref_model = None
        else:
    ...
    def _prepare_deepspeed(self, model: PreTrainedModelWrapper):
        ...
        # If ZeRO-3 is used, we shard both the active and opponent model.
        # Otherwise, we assume the opponent model fits in memory and is initialized on each device with ZeRO disabled (stage 0)
    ...
    def kto_loss(
        ...
        opponent_free: bool = False,
    ) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the SPIN loss for a batch of policy and opponent model log probabilities.
        ...
        Args:
            ...
            opponent_real_logps: Log probabilities of the opponent model for the real responses. Shape: (batch_size,)
            opponent_generated_logps: Log probabilities of the opponent model for the generated responses. Shape: (batch_size,)
            beta: Temperature parameter for the SPIN loss, typically something in the range of 0.1 to 0.5. We ignore the opponent model as beta -> 0.
            opponent_free: If True, we ignore the _provided_ opponent model and implicitly use a opponent model that assigns equal probability to all responses.
        ...
        if self.loss_type == "kto_pair":
            ...
            KL = (policy_real_logps+policy_generated_logps-opponent_generated_logps-opponent_real_logps).mean().detach().clamp(min=0)

            real_logratios = policy_real_logps - opponent_real_logps
            generated_logratios = policy_generated_logps - opponent_generated_logps
            # As described in the KTO report, the KL term for real (generated) is estimated using the generated (real) half.
            losses = torch.cat(
                (
            ...
            KL = (policy_real_logps+policy_generated_logps-opponent_generated_logps-opponent_real_logps).mean().detach().clamp(min=0)
            real_logratios = policy_real_logps - opponent_real_logps
            real_losses = (self.prior*(1 - F.sigmoid(self.beta * (real_logratios - generated_KL)))).mean()
    ...
    def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        """Generate samples from the model and opponent model for the given batch of inputs."""
        ...
        if self.ref_model is None:
            with self.accelerator.unwrap_model(self.model).disable_adapter():
                opponent_output = self.model.generate(
            ...
            opponent_output = self.ref_model.generate(
                batch["prompt_input_ids"],
                attention_mask=batch["prompt_attention_mask"],
                max_length=self.max_length,
                do_sample=True,
        ...
        opponent_output = pad_to_length(opponent_output, self.max_length, self.tokenizer.pad_token_id)
        opponent_output_decoded = self.tokenizer.batch_decode(opponent_output, skip_special_tokens=True)

        return policy_output_decoded, opponent_output_decoded
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        if torch.is_tensor(logs['grad_norm']):
            logs['grad_norm'] = logs['grad_norm'].cpu().item()
