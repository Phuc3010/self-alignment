...
class KTOTrainer(Trainer):
    ...
    def kto_loss(
        ...
        reference_real_logps: torch.FloatTensor,
        reference_generated_logps: torch.FloatTensor,
        ...
        Args:
            ...
            reference_real_logps: Log probabilities of the reference model for the real responses. Shape: (batch_size,)
            reference_generated_logps: Log probabilities of the reference model for the generated responses. Shape: (batch_size,)
            reference_KL_logps: Log probabilities of the reference model for the KL responses. Shape: (batch_size,)

        Returns:
            A tuple of four tensors: (losses, real_rewards, generated_rewards, KL).
        ...
        if self.loss_type=="kto_pair":
            if policy_real_logps.shape[0] != 0 or reference_real_logps.shape[0] != 0:
                ...
                real_losses = 1 - F.sigmoid(self.beta * (real_logratios - KL))
                real_rewards = self.beta * real_logratios.detach()
            else:
                # lists can't be empty -- if they are, then accelerate.gather will hang
                real_losses = torch.Tensor([torch.nan]).to(self.accelerator.device)
                real_rewards = torch.Tensor([torch.nan]).to(self.accelerator.device)

            if policy_generated_logps.shape[0] != 0 or reference_generated_logps.shape[0] != 0:
                generated_logratios = policy_generated_logps - reference_generated_logps
                generated_losses = 1 - F.sigmoid(self.beta * (KL - generated_logratios))
                generated_rewards = self.beta * generated_logratios.detach()
            else:
                # lists can't be empty -- if they are, then accelerate.gather will hang
                generated_losses = torch.Tensor([torch.nan]).to(self.accelerator.device)
                generated_rewards = torch.Tensor([torch.nan]).to(self.accelerator.device)

            losses = torch.cat(
                (self.desirable_weight * real_losses, self.undesirable_weight * generated_losses),
                0,
            )
        elif self.loss_type == "nnpu":
            have_positive = policy_real_logps.shape[0] != 0 or reference_real_logps.shape[0] != 0
            have_unlabeled = policy_generated_logps.shape[0] != 0 or reference_generated_logps.shape[0] != 0

            if have_positive and have_unlabeled:
                real_logratios = policy_real_logps - reference_real_logps
                real_losses = (self.prior*(1 - F.sigmoid(self.beta * (real_logratios - KL)))).mean()
                real_rewards = self.beta * real_logratios.detach()
            ...
            elif have_unlabeled and not have_positive:
                generated_logratios = policy_generated_logps - reference_generated_logps
                losses = (1 - F.sigmoid(self.beta * (KL - generated_logratios))).mean()
                generated_rewards = self.beta * generated_logratios.detach()
                real_rewards = torch.Tensor([torch.nan]).to(self.accelerator.device)

        return losses, real_rewards, generated_rewards, KL
    ...
    def get_batch_loss_metrics(
        ...
        if "reference_logps" in batch:
            real_idx = [i for i in range(batch["reference_logps"].shape[0]) if batch["label"][i] is True]
            generated_idx = [i for i in range(batch["reference_logps"].shape[0]) if batch["label"][i] is False]

            reference_real_logps = batch["reference_logps"][real_idx, ...]
            reference_generated_logps = batch["reference_logps"][generated_idx, ...]
            ...
            with torch.no_grad():
                if self.ref_model is None:
                    with self.accelerator.unwrap_model(self.model).disable_adapter():
                        (
                            reference_real_logps,
                            reference_generated_logps,
                            _,
                            _,
                            reference_KL_logps,
        ...
        losses, real_rewards, generated_rewards, kl = self.kto_loss(
            policy_real_logps,
            policy_generated_logps,
            policy_KL_logps,
            reference_real_logps,
