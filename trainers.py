...
class KTOTrainer(UnpairedPreferenceTrainer):
    def loss(self,
        ...
        dist.nn.all_reduce(KL, op=dist.ReduceOp.SUM) #TODO: Uncomment this
    ...
    def nnpu_loss(self,
        ...
        if policy_chosen_logps.shape[0] != 0:
            chosen_logratios = (policy_chosen_logps - reference_chosen_logps)
            expert_losses = 1 - F.sigmoid(self.config.loss.beta * (chosen_logratios - KL))
            expert_losses = self.config.loss.prior * expert_losses
            expert_negative_losses = 1 - F.sigmoid(self.config.loss.beta*(KL - chosen_logratios))
            expert_negative_losses = self.config.loss.prior*expert_negative_losses
            
            # policy_loss = torch.clamp(eta*trainer.get_batch_metrics(chosen_batch) - trainer.get_batch_metrics(unlabeled_batch), min=-0.0)
            chosen_rewards = self.config.loss.beta * chosen_logratios.detach()
        else:
            # important to cast to policy_dtype; otherwise error will occur during all_gather
            expert_losses = torch.Tensor([]).to(self.policy_dtype).to(self.device)
            expert_negative_losses = torch.tensor([]).to(self.policy_dtype).to(self.device)
            chosen_rewards = torch.Tensor([]).to(self.policy_dtype).to(self.device)
        
        if policy_rejected_logps.shape[0] != 0:
            unlabeled_negative_loss, _ = trainer.get_batch_metrics(unlabeled_batch)
            rejected_logratios = (policy_rejected_logps - reference_rejected_logps)
            rejected_losses = 1 - F.sigmoid(self.config.loss.beta * (KL - rejected_logratios))
            policy_losses = torch.clamp(expert_negative_losses-rejected_losses, min=-0.0)
            rejected_rewards = self.config.loss.beta * rejected_logratios.detach()
        else:
            # important to cast to policy_dtype; otherwise error will occur during all_gather
            rejected_losses = torch.Tensor([]).to(self.policy_dtype).to(self.device)
        ...
        losses = torch.cat((self.config.loss.desirable_weight * chosen_losses, self.config.loss.undesirable_weight * rejected_losses), 0)
        chosen_idx = [i for i in range(batch["target_combined_input_ids"].shape[0]) if batch['status'][i] == 'chosen']
        unlabeled_idx = [i for i in range(batch['target_combined_input_ids'].shape[0]) if batch['status'][i] == 'unlabeled']
    ...
    def nnpu_loss(self, batch):

        chosen_idx = [i for i in range(batch["target_combined_input_ids"].shape[0]) if batch['status'][i] == 'chosen']
        unlabeled_idx = [i for i in range(batch['target_combined_input_ids'].shape[0]) if batch['status'][i] == 'rejected']
        chosen_batch = {k:v.to("cuda:0")[chosen_idx,...] if torch.is_tensor(v) else [v[i] for i in chosen_idx] for k, v in batch.items() }
        unlabeled_batch = {k: v.to("cuda:0")[unlabeled_idx, ...] if torch.is_tensor(v) else [v[i] for i in unlabeled_idx] for k, v in batch.items()}

        real_loss, metrics = trainer.get_batch_metrics(chosen_batch)
        real_loss = eta*real_loss
        chosen_batch['status'] = ['rejected']*len(chosen_batch['status'])

        real_negative_loss, _ = trainer.get_batch_metrics(chosen_batch)
        real_negative_loss = real_negative_loss*eta
        unlabeled_negative_loss, _ = trainer.get_batch_metrics(unlabeled_batch)
        policy_loss = torch.clamp(real_negative_loss-unlabeled_negative_loss, min=-0.0)
        # policy_loss = torch.clamp(eta*trainer.get_batch_metrics(chosen_batch) - trainer.get_batch_metrics(unlabeled_batch), min=-0.0)
        losses = real_loss + policy_loss
...
if __name__=="__main__":
    ...
    from dataloader import UnpairedPreferenceDataLoader
    import json
    from pathlib import Path
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-1b-deduped")
    tokenizer.pad_token_id = tokenizer.eos_token_id

    train_loader = UnpairedPreferenceDataLoader(
        ["synthetic"], tokenizer, batch_size=2, n_examples=300, n_epochs=1, split="train"
    )
    eval_loader = UnpairedPreferenceDataLoader(
        ["synthetic"], tokenizer, batch_size=32, n_examples=300, n_epochs=1, split="test"
    )
    policy = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-160m-deduped", torch_dtype=torch.bfloat16, device_map="auto")
    config = OmegaConf.load("config/config.yaml")
    reference_model = AutoModelForCausalLM.from_pretrained("EleutherAI/pythia-160m-deduped", torch_dtype=torch.bfloat16, device_map="auto")
    trainer = KTOTrainer(
        tokenizer, 
        config,
        train_loader,
