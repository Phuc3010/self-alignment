...
epochs=("1" "2")
SCRIPT_COMMAND="ACCELERATE_LOG_LEVEL=info accelerate launch --config_file config/multi_gpu.yaml --num_processes=2 --main_process_port 2950 src/run_kto.py config/config.yaml --prior="
...
for epoch in "${epochs[@]}"; do
  for prior in "${priors[@]}"; do
    ...
    EVAL_PATH="data/output/zephyr-7b-kto-iter0-${prior}-${epocha}"
